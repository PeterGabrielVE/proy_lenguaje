...
{
    //
    public static function modelCustomersListWithPageNo($getPageNo = null){
		...
		$no_of_rows_result = \DB::select('select count(*) as count from customers');
		...
		$get_customers = \DB::select("SELECT * FROM customers ORDER BY ID DESC LIMIT $limit_to_start_from, $no_of_posts_to_show ");
		
    	return array('customers'=>$get_customers, 'get_no_of_pages_left'=>$pages_left, 'get_current_page_number'=>$get_page_number, 'get_next_page_number'=>$the_next_page_number, 'get_previous_page_number'=>$the_previous_page_number, 'get_showing_start_at'=>$limit_to_start_from, 'get_showing_end_at'=>$get_showing_end_at, 'total_number_of_entries'=>$pages_count);
    }

    public static function modelCustomersGeneralSearchWithPageNo($getPageNo = null, $sqlcode = null, $sql_count_code = null){
    	if ( (isset( $getPageNo )) && (!empty( $getPageNo )) && (intval( $getPageNo ) > 0 ) ){
			$get_page_number = intval($getPageNo);
		} else {
			$get_page_number = 1;
		...
		$get_showing_end_at = $limit_to_start_from + 50;
		//$sql_code_to_use = 
		$get_customers = \DB::select($sqlcode . " ORDER BY ID DESC LIMIT $limit_to_start_from, $no_of_posts_to_show");
